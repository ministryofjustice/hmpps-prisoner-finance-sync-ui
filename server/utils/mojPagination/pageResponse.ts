...
export default class PageResponse {
  ...
  pageRange(pagesBefore: number, pagesAfter: number): number[] {
    const idealStartPage = this.page - pagesBefore
    const idealEnd = this.page + pagesAfter
    if (idealStartPage < 1) {
  ...
  resultsFrom(): number {
    return Math.min(this.totalElements, (this.page - 1) * this.size + 1)
  }

  resultsTo(): number {
    return Math.min(this.totalElements, (this.page - 1 + 1) * this.size)
  }
